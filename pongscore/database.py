...
class Database:
    ...
    def leaderboard(self):
        lb = self.supabase.table('users').select('name', 'elo', 'wins', 'loses').order('elo', desc=True).execute()
        return lb.data
    ...
    def match_history(self):
        history = self.supabase.table('matches').select("created_at, winner, loser, winner_score, loser_score").execute()
        return history.data

    def get_player_info(self, userid):
        elo = self.supabase.table('users').select('wins', 'loses', 'elo').eq("id", userid).execute()
        return elo.data[0]
